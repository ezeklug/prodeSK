...
export interface IPartidosClient {
    getPartidos(): Observable<PartidoDto[]>;
}

@Injectable({
    providedIn: 'root'
})
export class PartidosClient implements IPartidosClient {
    ...
    getPartidos(): Observable<PartidoDto[]> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetPartidos(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetPartidos(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<PartidoDto[]>;
                }
            } else
                return _observableThrow(response_) as any as Observable<PartidoDto[]>;
        }));
    }

    protected processGetPartidos(response: HttpResponseBase): Observable<PartidoDto[]> {
        ...
        if (status === 200) {
            ...
            if (Array.isArray(resultData200)) {
                ...
                for (let item of resultData200)
                    result200!.push(PartidoDto.fromJS(item));
            }
            else {
                result200 = <any>null;
            }
...
export interface IPronosticosClient {
    guardarPronosticos(command: GuardarPronosticosCommand): Observable<boolean>;
}

@Injectable({
    providedIn: 'root'
})
export class PronosticosClient implements IPronosticosClient {
    private http: HttpClient;
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    guardarPronosticos(command: GuardarPronosticosCommand): Observable<boolean> {
        let url_ = this.baseUrl + "/api/Pronosticos";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGuardarPronosticos(response: HttpResponseBase): Observable<boolean> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
...
export class GuardarPronosticosCommand implements IGuardarPronosticosCommand {
    pronosticos?: PronosticoDto[];

    constructor(data?: IGuardarPronosticosCommand) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(_data?: any) {
        if (_data) {
            if (Array.isArray(_data["pronosticos"])) {
                this.pronosticos = [] as any;
                for (let item of _data["pronosticos"])
                    this.pronosticos!.push(PronosticoDto.fromJS(item));
            }
        }
    }

    static fromJS(data: any): GuardarPronosticosCommand {
        data = typeof data === 'object' ? data : {};
        let result = new GuardarPronosticosCommand();
        result.init(data);
        return result;
    }
    ...
    toJSON(data?: any) {
        data = typeof data === 'object' ? data : {};
        if (Array.isArray(this.pronosticos)) {
            data["pronosticos"] = [];
            for (let item of this.pronosticos)
                data["pronosticos"].push(item.toJSON());
        }
        return data;
    }
}

export interface IGuardarPronosticosCommand {
    pronosticos?: PronosticoDto[];
}

export class PronosticoDto implements IPronosticoDto {
    partidoId?: number;
    resultado?: number;

    constructor(data?: IPronosticoDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(_data?: any) {
        if (_data) {
            this.partidoId = _data["partidoId"];
            this.resultado = _data["resultado"];
        }
    }

    static fromJS(data: any): PronosticoDto {
        data = typeof data === 'object' ? data : {};
        let result = new PronosticoDto();
        result.init(data);
        return result;
    }
    ...
    toJSON(data?: any) {
        data = typeof data === 'object' ? data : {};
        data["partidoId"] = this.partidoId;
        data["resultado"] = this.resultado;
        return data;
    }
}

export interface IPronosticoDto {
    partidoId?: number;
    resultado?: number;
}
